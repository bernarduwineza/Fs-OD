...
\label{tab:table1}
\end{table}
In general increasing the number of shots (i.e. data samples for the novel classes) helps in improving the detection results. This can be observed from figures \ref{fig:coco_results}, \ref{fig:pascal_results}, \ref{fig:kitti_plots}. In case of COCO we observed that the detector was producing a lot of bounding boxes with greater than 50\% prediction scores.\par
...
\begin{figure}[h!]
  ...
  \subcaption{10-shot}
  \end{minipage}
  \caption{Detection Results on PASCAL dataset for different number of shots }
